...
当前总状态：**Fish 专用引擎已接入 IGESS；鱼厅金钱和垃圾佬材料使用统一在线/离线原子结算，已装备杠铃仅在互斥前台行为 `exercise_barbell` 在线执行时产出力量，离线力量为零。默认画像已配置每天在线 2 小时、离线 22 小时；杠铃合成与鱼厅升级为权重 `100` 的高优先级行为，鱼升级为权重 `0.1` 的低优先级行为且仅在最低升级价严格低于当前材料 `1/10` 时执行，升级价格从材料余额扣除。两类重生使用同一条硬优先级规则：只要当前达到任一重生要求，就只从可执行重生中选择并立即进行。废料加工、两类重生及离线 checkpoint 已接通；24h 离线上限、鱼雷购买、新境界突破、临时效果和长期策略模拟仍未完成**
...
| 正式经济闭环 | `[~]` | 鱼厅/金钱、废料/材料、在线历史境界追赶、材料→鱼厅容量、材料→杠铃→主动锻炼力量、两类重生永久倍率及离线基础结算已接通；鱼雷购买、新境界突破和未确认离线扩展尚未闭合 | Phase 4–8 |
...
+ FishEconomySimulator 已接入生产数据驱动的投掷、鱼厅金钱、废料材料、在线历史境界追赶、摸鱼厅升级、杠铃合成/装备/主动在线锻炼、两类重生永久倍率和可选加权行为循环
+ 默认画像 `daily_online_seconds=7200`，每天在线 2 小时、离线 22 小时；`synthesize_barbell / upgrade_fish_hall` 权重均为 `100`，`manual_throw / exercise_barbell` 权重均为 `1`，`upgrade_fish` 权重为 `0.1`；鱼升级仅选择最低价且要求价格严格低于当前材料 `1/10`，成功时扣除材料
+ 两类重生均已进入生产画像，达到要求时硬优先于全部普通前台行为
+ 24h 上限、双倍领取、临时效果、新境界突破和鱼雷购买尚未完成
```
...
状态：`[~]`，主动投掷、加权行为事件循环、后台鱼厅金钱/垃圾佬材料、主动杠铃锻炼、每日在线/离线边界及在线历史境界追赶均已接入；新境界突破和 buff 等领域事件仍待实现。

- `[~]` `TimeEngine` 已提供 `(start, end]` 绝对周期事件边界，鱼厅、废料加工和历史境界在线追赶已解析跳跃结算；新境界突破等后续领域事件跳跃契约仍待扩展。
- `[~]` 加权模式按“当前在线模式的后台金钱/材料与当前前台锻炼力量结算→前台行为完成→会话边界→timeline 采样→选择下一行为”稳定排序；突破和 buff 等新增领域事件优先级仍待定义。
- `[x]` 通用 `BehaviorScheduler` 已实现玩家级权重、固定/整数均匀时长、可用性过滤、目标选择、`idle` 和三路稳定 RNG。
- `[x]` Fish 已接入 `manual_throw / upgrade_fish / upgrade_fish_hall / synthesize_barbell / exercise_barbell / strength_rebirth / trash_man_rebirth / idle`；fixture 已验证互斥行为、持续时长、杠铃仅锻炼产出、目标策略、两类重生和行为中 checkpoint。生产 `default` 已启用权重 `100` 的优先 `synthesize_barbell / upgrade_fish_hall`、权重 `1` 的 `manual_throw / exercise_barbell`、权重 `0.1` 的低优先级 `upgrade_fish`，以及固定 `1` 秒的两类重生。杠铃与鱼厅升级按可支付性过滤；鱼升级只选择最低价格并要求其严格低于当前材料 `1/10`；未拥有杠铃时训练自动过滤。任一重生达到要求时硬优先于全部普通行为；两种同时满足时先稳定选择一种，下一轮立即执行另一种。
- `[~]` 已支持废料加工和不超过历史最高境界的在线自动修炼；新境界瓶颈、资助突破和 buff 过期事件尚未实现。
- `[x]` 旧主动投掷模式支持事件边界 checkpoint；加权行为模式可在行为中间 checkpoint，恢复时继续已保存行为而不重抽。
- `[~]` 已验证在线训练后进入离线的连续运行与离线中 checkpoint 分段恢复一致；完整连续 24 小时与两个 12 小时正式画像运行仍待配置。
- `[ ]` 验证 tick 与事件跳跃模式结果一致。

### Phase 4：鱼、摸鱼厅和金钱闭环

状态：`[~]`，固定最高收益编队、等级收入、容量、金钱结算、鱼升级材料扣款和 trace 已接入；生产画像已启用手动炸鱼及低优先级最低价格鱼升级，出售口径与正式长时验证仍待完成。
...
状态：`[~]`，废料库存、在线/离线批量加工、材料产出、境界速度、转世材料倍率、仅在线历史境界追赶和 checkpoint 已接入；新境界瓶颈和资助突破尚未实现。
...
状态：`[~]`，材料升级鱼/摸鱼厅容量与材料合成杠铃/主动在线锻炼力量均已完成生产表适配、原子扣款、互斥行为接入和 checkpoint；鱼雷购买与垃圾佬新境界突破仍受正式数据或规则缺口阻塞。
...
- `[~]` 生产画像已将杠铃合成配置为权重 `100` 的优先行为，并使用 `random_affordable` 选择当前可支付且未拥有的目标；鱼雷购买、等待时间和回本报告尚未实现。
- `[~]` 已验证鱼升级、鱼厅升级和杠铃合成均严格消费材料且失败不修改状态；鱼雷购买和垃圾佬突破等待对应规则完成。

验证证据：原 Phase 6 回归覆盖鱼厅当前行价格、BigNumber 扣款、末行零值哨兵、容量严格递增、杠铃生产表 15 档与 `timeCost`、库存数量不放大、材料不足失败不修改状态、`fixed_max_income` 扩容重排及行为中 checkpoint；2026-07-24 的 Phase 8 回归进一步纠正并覆盖“仅 `exercise_barbell` 产出力量、其他行为与离线均为零”。

### Phase 7：重生和永久成长

状态：`[~]`，两类重生事务、永久总倍率、无目标行为、在线历史境界追赶、checkpoint 和生产硬优先级策略已完成；长期回本分析尚未实现。
...
- `[ ]` 在正式 24h/长期场景中输出重生时间线与回本分析。
- `[x]` 已验证力量重生只重置 `wallet.strength`；垃圾佬转世只重置当前境界和修炼进度，历史最高境界、加工队列和其他数值状态保留。

验证证据：新增 `tests/test_fish_rebirth_priority.py` 验证两类重生同时满足时会连续抢占全部普通行为；当前 Fish 全量定向回归显式排除既有 RNG CLI 子进程环境用例后为 `104 passed, 4 deselected`。既有回归覆盖两张一基重生表契约、表外默认 `1×`、门槛不足/满档/突破中失败不修改状态、旧倍率先结算、永久总倍率 trace、重置/保留字段、转世后在线历史境界追赶、无目标行为可用性以及行为中 checkpoint 连续/分段恢复等价。

### Phase 8：离线、临时效果和长期模拟

状态：`[~]`，每日在线预算、在线/离线事件边界、离线鱼厅/废料加工和防重复 checkpoint 首片已完成；24h 上限、双倍领取和临时效果尚未确认或实现。
...
验证证据：`tests/test_fish_offline_sessions.py` 覆盖默认 2h/22h 配置契约、在线训练严格截止、离线力量为零、鱼厅/废料加工 50%、离线不推进修炼、每日窗口纯函数以及离线中 checkpoint 连续/分段等价；显式排除既有 RNG CLI 子进程环境用例后的 Fish 全量定向回归为 `102 passed, 4 deselected`。
...
- `[!]` 垃圾佬突破费用/瓶颈规则仍不完整；杠铃的主动在线锻炼周期与材料价格已由当前生产 `tbbarbell` 完整表达，离线力量已确认并实现为 `0`。
...
当前里程碑：**Phase 8 离线基础首片以及杠铃、鱼厅、鱼升级、两类重生生产画像已完成。默认画像 `daily_online_seconds=7200`，每天在线 2 小时、离线 22 小时；杠铃合成和鱼厅升级为权重 `100` 的高优先级行为，炸鱼和锻炼权重均为 `1`，鱼升级为权重 `0.1` 的低优先级行为，上述普通行为固定 `1` 秒。两类重生也固定 `1` 秒，但使用硬优先级：只要当前满足任一重生要求，就只从可执行重生中选择；两种同时满足时连续完成后才恢复普通行为。鱼升级只选择最低升级价格，同价按 `instanceId` 升序决胜，并要求价格严格低于当前材料 `1/10`，成功时从材料余额扣款；各行为不可执行时自动过滤。离线时摸鱼厅金钱和垃圾佬废料加工按 50% 结算，杠铃力量为 0，垃圾佬修炼不推进；在线/离线边界共享 `lastSettledAt`，离线中 checkpoint 恢复不重复领取。下一步是验证有完整成长选择的 24h/长期场景并输出重生回本时间线；24h 离线上限、双倍领取和临时效果仍保持阻塞。**
...
7. `[~]` 固定 `max_income` 自动上阵、等级鱼厅收入、鱼升级材料消费、最低价/材料 `1/10` 生产升级画像和生产手动炸鱼已完成；等待正式长时场景验证与出售口径后闭合 Phase 4。
8. `[~]` 废料聚合库存、批量加工、境界速度、材料产出、在线历史境界追赶与 checkpoint 已完成；新境界瓶颈和资助突破待正式规则。
9. `[~]` 交叉升级、两类重生和离线基础已完成材料→鱼厅容量、材料→杠铃→主动在线锻炼力量、力量重生→鱼厅永久总倍率和垃圾佬转世→材料永久总倍率；默认画像已配置 2h/22h 作息、高优先级杠铃合成/鱼厅升级、低优先级最低价鱼升级，以及“达到要求立即重生”的两类重生硬优先级。鱼雷购买与垃圾佬突破受正式字段/语义阻塞；下一步验证 24h/长期场景，再进入策略比较、KPI 和调参回归。
