...
| 正式经济闭环 | `[~]` | 鱼厅/金钱、废料/材料、在线历史境界追赶、材料→鱼厅容量、材料→杠铃→主动锻炼力量、金钱→鱼雷→TrashLuck、两类重生永久倍率及离线基础结算已接通；新境界突破和未确认离线扩展尚未闭合 | Phase 4–8 |
...
+ FishEconomySimulator 已接入生产数据驱动的投掷、鱼厅金钱、废料材料、在线历史境界追赶、摸鱼厅升级、鱼雷购买/自动装备、杠铃合成/装备/主动在线锻炼、两类重生永久倍率和可选加权行为循环
+ 默认画像 `daily_online_seconds=7200`，每天在线 2 小时、离线 22 小时；`purchase_torpedo / synthesize_barbell / upgrade_fish_hall` 权重均为 `100`，`manual_throw / exercise_barbell` 权重均为 `1`，`upgrade_fish` 权重为 `0.1`；鱼升级仅选择最低价且要求价格严格低于当前材料 `1/10`，成功时扣除材料
+ 两类重生均已进入生产画像，达到要求时硬优先于全部普通前台行为
+ 24h 上限、双倍领取、临时效果、新境界突破和 10/11 号长期价格验证尚未完成
```
...
- `[x]` Fish 已接入 `manual_throw / upgrade_fish / upgrade_fish_hall / purchase_torpedo / synthesize_barbell / exercise_barbell / strength_rebirth / trash_man_rebirth / idle`；fixture 已验证互斥行为、持续时长、杠铃仅锻炼产出、鱼雷纯价格购买、目标策略、两类重生和行为中 checkpoint。生产 `default` 已启用权重 `100` 的优先 `purchase_torpedo / synthesize_barbell / upgrade_fish_hall`、权重 `1` 的 `manual_throw / exercise_barbell`、权重 `0.1` 的低优先级 `upgrade_fish`，以及固定 `1` 秒的两类重生。鱼雷按 `highest_affordable` 只选择未拥有、比当前装备强且金钱可支付的最高档，不读取当前/历史力量；杠铃与鱼厅升级按可支付性过滤；鱼升级只选择最低价格并要求其严格低于当前材料 `1/10`；未拥有杠铃时训练自动过滤。任一重生达到要求时硬优先于全部普通行为；两种同时满足时先稳定选择一种，下一轮立即执行另一种。
...
- `[x]` 调价前 24h/7d 确定性基线已建立；调价后正式 run
  `20260725T040202603439Z-day_1_growth` 与
  `20260725T040259487026Z-week_1_growth` 使用模型摘要
  `sha256:66017a648f3ccce9ddb100a6e14e6dd54c0ee9e85177172e6859195c9ea42674`。
  7d TrashLuck 从 3 增长到 30，每日双 Luck 峰值最大相对误差 `14.83%`；
  系统级最大/尾部在线空窗降为 `14,330s / 7,199s`。
- `[ ]` 输出鱼、废料、金钱和材料每小时产出。
...
- `[~]` 首轮价格调参已完成；需用长期场景验证 10、11 号鱼雷，并与人类确认
  是否接受 power 档位导致的中段约 15% 误差，再进入大规模参数扫描。
...
当前里程碑：**Phase 8 离线基础首片以及杠铃、鱼厅、鱼升级、两类重生生产画像已完成。默认画像 `daily_online_seconds=7200`，每天在线 2 小时、离线 22 小时；杠铃合成和鱼厅升级为权重 `100` 的高优先级行为，炸鱼和锻炼权重均为 `1`，鱼升级为权重 `0.1` 的低优先级行为，上述普通行为固定 `1` 秒。两类重生也固定 `1` 秒，但使用硬优先级：只要当前满足任一重生要求，就只从可执行重生中选择；两种同时满足时连续完成后才恢复普通行为。鱼升级只选择最低升级价格，同价按 `instanceId` 升序决胜，并要求价格严格低于当前材料 `1/10`，成功时从材料余额扣款；各行为不可执行时自动过滤。离线时摸鱼厅金钱和垃圾佬废料加工按 50% 结算，杠铃力量为 0，垃圾佬修炼不推进；在线/离线边界共享 `lastSettledAt`，离线中 checkpoint 恢复不重复领取。生产 24h/7d 长时基线和重生直接产出/重置进度恢复时间线已经完成；下一步进入 Phase 9 的同 RNG 禁用重生反事实分叉、KPI 定义和策略比较。垃圾佬第二次转世仍被缺失的新境界突破规则阻塞，24h 离线上限、双倍领取和临时效果也继续保持阻塞。**

Phase 9 当前新增里程碑：**核心强度与永久养成已经接入正式
JSON/CSV/HTML，并完成 24h/7d 基线。下一步先与人类确认全部永久进展和
系统级进展各自的间隔、密度与 Luck 停滞阈值，再进入同 RNG 策略分叉、
compare/gate/scan 和正式参数扫描。**
...
9. `[~]` 交叉升级、两类重生、离线基础和金钱→鱼雷→TrashLuck 已完成；
   ...
   24h/7d 正式调价基线已完成，下一步验证 10、11 号长期价格、垃圾佬突破
   规则及 compare/gate/scan。
