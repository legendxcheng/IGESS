...
- 同源 1／7／30 天场景恢复每累计在线 5 分钟采样，另记录上下线、离线领取和重生前后状态。通过现有统一源会话查询，不改变游戏业务逻辑。
- 30 天新运行 `20260925T025501064040Z-month_1_growth`：837 个样本（原 31），13 次重生前后状态完整；全部业务回执和最终数值与旧运行一致，0 次拒绝。完整正式工作流 598.696 秒，其中源执行及通信 591.387 秒，网页生成 0.541 秒。
- 当前源码指纹与旧运行不同，生产导表不变；不把本次耗时差异视为严格性能对照。细节与可复核计时见 [采样恢复验证](../../.scratch/fish-source-report-readability/verification.md)。
- 修复资源图表遗漏：新增“资源（材料）余额”独立图表，支持含零线性视图与对数视图；更新现有 30 天报告，837 条资源采样完整显示，模拟产物保持不变。报表相关 33 项检查通过。
...
## 0. 2026-09-21 杠铃半秒收益累计

- [x] 对齐游戏 `fab55c16` 的小数 `Barbell.timeCost`：以 Decimal 保留毫秒精度，
  拒绝零、负数、非有限数和超出毫秒精度的值，继续只读权威生产导表。
- [x] 用户确认保留 60 秒锻炼行为，按 `strengthPerExercise / timeCost × elapsed`
  累计收益；当前 0.5 秒对应每段 120 次。全局整数秒时钟、策略边界与存档格式不变。
- [x] 半秒收益、画像倍率、非锻炼/离线零收益和行为中 checkpoint 重放通过验证；
  Fish/行为/checkpoint/时钟组合回归 322 passed，生产 1d 正式运行成功。
- 证据与报告见 [半秒杠铃验证](../../.scratch/fish-barbell-subsecond/verification.md)。
  这是周期收益支持验证；当前生产快照还有其他数值变化，不将新旧成长差异单独归因于半秒改动。

### 前序：2026-09-16 卖鱼材料收入

- [x] 对齐游戏 `1ddeebc1`：普通背包鱼售价为含变异一级产出 × 10，原子入材料；上阵鱼与神兽拒卖，不返升级金币。
- [x] 两个画像保护上阵鱼和品质前 X，累计在线 300 秒批量整理、耗时三秒；沿用重生和立即突破的优先级。
- [x] 无可卖鱼跳过、下线边界顺延、错过周期合并；冻结批次及周期支持 checkpoint，累计捕获与当前库存分离。
- [x] 完整与紧凑事件保留出售明细，报告区分卖鱼和加工材料，出售不计入永久进展。
- [x] 全量与定向测试、生产四场景及账本审计完成，证据见
  [verification.md](../../.scratch/fish-sale-material/verification.md)；数值结果见
  [卖鱼材料基线](reports/fish-sale-material-baseline.md)。
- 首月卖鱼提供约 3.36% 材料毛收入，系统进展 87 次，仍有四个在线日无系统进展。

### 前序：2026-09-16 金币升级迁移
...
| 正式经济闭环 | `[~]` | 鱼厅/金钱、废料及卖鱼/材料、历史境界追赶、材料突破、材料→鱼厅/鱼雷、金钱→普通鱼升级/杠铃、两类永久倍率及离线基础结算已接通；未确认离线扩展尚未闭合 | Phase 4–8 |
...
- `[x]` 鱼出售价格已核对实装：含变异一级产出 × 10，入材料；周期清理策略与正式四场景见本页最新记录。
...
- `[x]` 已实现投掷领取、废料入库、统计、普通鱼金币升级、出售材料及鱼厅/加工统一结算事务；出售区分当前库存与累计捕获并保留原子性，图鉴不属于模拟范围。
...
状态：`[x]`，普通鱼固定最高收益编队、金币升级、卖鱼材料、行为策略和账本已接入；当前版本 smoke/1d/7d/30d 正式验证完成。本阶段不包含神兽。

目标链路：`炸鱼 -> 鱼实例 -> 自动最高收益上阵 -> 鱼厅产出金钱`、`金币 -> 鱼升级 -> 更高鱼厅收入`，以及 `闲置普通鱼 -> 出售 -> 材料`。

- `[x]` 已实现鱼背包、领取、升级、出售和固定最高收益鱼厅栏位事务。
- `[x]` 接入基础金钱产出、等级 `×1.25^(n-1)` 曲线与变异收入倍率。
- `[x]` 已接入鱼升级金币价格 `B×变异倍率×1.5^(n-1)`、BigNumber 金币扣款、100 级上限和升级后自动重排。生产策略为品质前 X 与已上阵交集中的最低等级优先，权重 1、三秒一级，保留下一把有效杠铃的攒钱估计；不使用旧材料十分之一门槛。
...
- `[x]` 鱼出售价格、入材料及周期清理策略已实现并通过正式生产运行；完整证据见卖鱼材料基线。
...
当前里程碑：普通鱼金币升级、卖鱼材料策略、账本与报告已完成，正式 1d/7d/30d 系统永久
进展为 `18 / 48 / 87`，单鱼升级与卖鱼排除。现有目标区间 `8..12 / 16..24 / 32..48`
下三阶段均偏快；这是同快照新基线的观测，不能沿用旧 `9 / 23 / 36` 的通过结论。
月内仍有 4 个在线日无系统进展，最长 TrashLuck 停滞约 13.74 在线小时。

下一步：基于本轮基线研究成长密度与长期停滞；实现同 RNG 策略分叉和 KPI 的
compare/gate/scan 消费；独立修复 Fish 自动探针；继续明确神兽收益、
...
7. `[x]` 固定 `max_income` 自动上阵、等级鱼厅收入、普通鱼金币消费、品质前 X 上阵交集培养、周期卖鱼材料及 smoke/1d/7d/30d 正式验证已完成，普通鱼 Phase 4 闭合。
