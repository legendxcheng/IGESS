...
普通鱼出售获得**材料（游戏界面的资源）**，和废料加工共用 `wallet.material`。
单鱼售价为 `P0 × 10`，`P0` 是含变异的一级产出，不乘等级、重生、离线或画像倍率，
不返还升级金币。倍率 10 对齐实际游戏 `FishGameplayConfig.fishSellIncomeSeconds`，
是有版本记录的规则常量，写入正式 manifest；表内基础产出和变异倍率仍只读生产快照。
上阵鱼和神兽不可出售；该命令没有“品质前 X”限制，保留组属于玩家策略。

两个现有画像均启用 `sell_fish / keep_deployed_and_top_quality`：保留当前上阵鱼和
全背包品质前 X 条普通鱼，卖掉其他普通鱼。X 沿用当前普通鱼名额，同品质先保留
等级较高者，再按实例 ID 升序；不额外保留未来扩容备用鱼或已有升级投入的落选鱼。
每累计在线 300 秒产生整理时机，固定占用三秒，期间仍结算后台生产；不打断当前动作，
重生和可执行立即突破优先，随后到期出售优先于其他普通行为。没有可卖鱼则跳过，
不足三秒则顺延下次上线，多个延误周期合并一次；缺材料不提前出售。
出售开始时冻结整批实例，完成时原子到账，周期与批次均支持 checkpoint 恢复。
累计捕获数不随出售回减，实例 ID 不回收；缓存通过新的库存列表重新建立。

报告分别展示卖鱼和加工材料，在线曲线仍排除离线加工，收入卡片的加工总额包含离线。
卖鱼本身不计入永久进展。确认方案见
[出售设计](../../.scratch/fish-sale-material/design.md)，新基线见
