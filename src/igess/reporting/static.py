...
def _html(inline_payload: str, title: str) -> str:
    return "\n".join(
        [
            ...
            "      <h2>Core Strength Progression</h2>",
            '      <p class="section-note">Strength and dual-Luck values use cumulative active play time; peaks remain visible across rebirth resets.</p>',
            '      <div data-fish-core-kpis class="kpi-grid" role="list" aria-label="Core strength progression"></div>',
            '      <div id="core-strength-chart" class="chart"></div>',
            '      <div id="luck-progression-chart" class="chart"></div>',
            "    </section>",
            '    <section class="band" data-fish-persistent-section hidden>',
            "      <h2>Persistent Progression Events</h2>",
            '      <p class="section-note">Only cross-fish or permanent capacity gains are counted. Single-fish upgrades and temporary buffs are excluded.</p>',
            '      <div data-fish-persistent-kpis class="kpi-grid" role="list" aria-label="Persistent progression"></div>',
            '      <div id="progression-density-chart" class="chart"></div>',
