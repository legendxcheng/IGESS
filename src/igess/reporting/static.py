...
def _html(inline_payload: str, title: str) -> str:
    return "\n".join(
        [
            ...
            "    </section>",
            '    <section class="band" id="source-material" data-source-material-section hidden>',
            "      <h2>资源（材料）余额</h2>",
            '      <p class="section-note">游戏中的“资源”即材料，用于鱼厅扩容、鱼雷购买和境界突破。曲线为源状态记录的可用余额，已扣除消耗，不代表累计获取量。横轴累计在线时间；可缩放查看局部。线性视图保留零值，对数视图仅显示大于零的余额。</p>',
            '      <div class="controls" aria-label="资源余额坐标轴">',
            '        <button type="button" data-source-material-scale="linear" class="active">线性（含零）</button>',
            '        <button type="button" data-source-material-scale="log">对数（看数量级）</button>',
            "      </div>",
            '      <div id="source-material-chart" class="chart"></div>',
            "    </section>",
            ...
            '      <h3 class="chart-list-heading">每日明细</h3>',
